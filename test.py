...
class AudioPipeline(torch.nn.Module):
    def __init__(
        ...
        self.mel_scale = T.MelScale(
            n_mels=n_mel, sample_rate=freq, n_stft=n_fft // 2 + 1
        )
...
def load_remote():
    return torch.hub.load(
        "vtuber-plan/hifi-gan:v0.2.1", "hifigan_48k", force_reload=True
    )
...
audio_pipeline = AudioPipeline(
    freq=48000, n_fft=2048, n_mel=128, win_length=2048, hop_length=512
)

...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output", type=str, default="./filelists", help="File list output path"
    )
    ...
    with open(
        os.path.join(args.output, origin_filename.replace(".txt", "_valid.txt")),
        "w",
        encoding="utf-8",
    ) as f:
    ...
    with open(
        os.path.join(args.output, origin_filename.replace(".txt", "_test.txt")),
        "w",
        encoding="utf-8",
    ) as f:
